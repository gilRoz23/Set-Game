...
public class Table {
    ...
    protected List<List<Integer>> tokens;

    protected Object allPlayersLock = new Object(); // lock for all players

    protected boolean allCanRecieveKey = true;
    ...
    public Table(Env env, Integer[] slotToCard, Integer[] cardToSlot) {
        ...
        tokens = new ArrayList<>(env.config.players); // init tokens nested list
...
//        initiate which slots in table are empty
     ...
     */
    public synchronized boolean placeToken(int playerId, int slot) { // changed to boolean

        if (slot == Dealer.IRRELEVANT_SLOT | emptySlots.contains(slot)) { // if the slot he holds is not relevant anymore
            return false;
        } else {
            if (tokens.get(playerId).contains(slot)) { // the token needs to be removed
                removeToken(playerId, slot);
                return false;

            } else { // I need to place the token

                if (tokens.get(playerId).size() >= Dealer.SET_SIZE) // if i already have 3, and want to add another token - dont do anything.
                    return false;

                tokens.get(playerId).add(slot); // add the slot to player's tokens
                env.ui.placeToken(playerId, slot);

                if (tokens.get(playerId).size() < Dealer.SET_SIZE) { // case where there are 1 or two tokens now.
                    return false;
                }
                // add the set to the dealer's queue
                if (tokens.get(playerId).size() == Dealer.SET_SIZE) {
    ...
    public boolean removeToken(int playerId, int slot) {
        ...
        return tokens.get(playerId).remove(intSlot); // returns true iff removed successfully
