...
//                terminate = true;
    ...
    private void removeCardsFromTable(List<Integer> slotsToRemove, boolean removeForever) { // remove for a valid set
        while (!slotsToRemove.isEmpty()) {
            ...
            for (Player curr_player : players) {
                while (curr_player.playerQueue.contains(slotToRemove)) {
                    curr_player.playerQueue.remove(slotToRemove); // if the slot is in the queue of the player - remove it.
                }
    ...
    private void placeCardsOnTable() {
        ...
        while (!table.emptySlots.isEmpty() && !table.deck.isEmpty()) {
    ...
    private void sleepUntilWokenOrTimeout() {
        ...
        try {
            ...
            Integer playerId;
            playerId = table.setsForDealer.poll(sleepTime, TimeUnit.MILLISECONDS);

            if (playerId != null) { // woke up because of a set
                setFromQueue = table.tokens.get(playerId);
                if (setFromQueue.size() == SET_SIZE) { // size of set is ok. (no token was removed from the set)
                    ...
                    if (setIsValid) { // set is valid
                        ...
                        while(!table.tokens.get(playerId).isEmpty()){
                            table.removeToken(playerId, table.tokens.get(playerId).get(0));
                        }
